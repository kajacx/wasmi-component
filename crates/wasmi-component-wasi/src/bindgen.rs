...
pub fn add_root_to_linker<T: RootImports>(linker: &mut Linker<T>) -> Result<(), LinkerError> {
    ...
    linker.func_typed::<(i32, u64), Result<Vec<u8>, StreamError>>(
    ...
    linker.func_typed::<(), Vec<(String, String)>>(
        "wasi:cli/environment@0.2.0",
        "get-environment",
        |host_data, params| host_data.get_environment(),
    )?;

    linker.func_typed::<(Result<(), ()>,), ()>(
        "wasi:cli/exit@0.2.0",
        "exit",
        |host_data, params| host_data.exit(params.0),
    )?;

    linker.func_typed::<(), i32>("wasi:cli/stdin@0.2.0", "get-stdin", |host_data, params| {
        host_data.get_stdin()
    })?;
