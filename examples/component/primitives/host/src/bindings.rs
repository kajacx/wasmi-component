...
pub fn instantiate_example_world(
    ...
    let cleanup_func = instance
        .get_typed_func::<i32, ()>(
            ctx.as_context_mut(),
            "cabi_post_wasmi-component:examples/funcs@0.1.0#add-s32",
        )
        .ok();
    ...
    let module_func = instance
        .get_func(
            ctx.as_context_mut(),
            "wasmi-component:examples/funcs@0.1.0#greet",
        )
        .unwrap();
    let cleanup_func = instance
        .get_typed_func::<i32, ()>(
            ctx.as_context_mut(),
            "cabi_post_wasmi-component:examples/funcs@0.1.0#greet",
        )
        .ok();
    ...
    let cleanup_func = instance
        .get_typed_func::<i32, ()>(ctx.as_context_mut(), "cabi_post_add-u32")
        .ok();
    let add_u32 = TypedFunc::new(memory_pre.clone(), module_func, cleanup_func);

    let module_func = instance
        .get_func(ctx.as_context_mut(), "additional#add-f32")
        .unwrap();
    let cleanup_func = instance
        .get_typed_func::<i32, ()>(ctx.as_context_mut(), "cabi_post_additional#add-f32")
        .ok();
